...
class SimpleGAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, edge_dim, num_heads=1):
        ...
        # Get output dim for GatConv-Layers
        self.gat_output_dim = hidden_channels // num_heads

        # Graph Attention Layers
        self.conv1 = GATConv(
            in_channels=hidden_channels,
            out_channels=self.gat_output_dim,
...
dummy_x = torch.randn(size=(num_nodes, node_dim))                               # Shape: [5, 9]
dummy_edge_index = torch.randint(low=0, high=num_nodes, size=(2, num_edges))    # Shape: [2, 8]
dummy_edge_attr = torch.randn(size=(num_edges, edge_dim))                       # Shape: [8, 3]
dummy_batch = torch.randint(low=0, high=batch_size, size=(num_nodes,))          # Shape: [5]
...
# save trained model
torch.save(model, '../deep_learning_outputs/trained_models/basic_gat_v03.pt')
...
plot_history(
    ...
    figsize=(16, 8),
    file='../deep_learning_outputs/figures/train_history_basic_gat_v03.png'
)
...
training_lod_df.to_csv(
    '../deep_learning_outputs/training_logs/train_history_basic_gat_v03.csv',
    index=False,
    header=True
)
...
test_metrics_csv = '../deep_learning_outputs/model_evaluation/basic_gat_test_evaluation_v03.csv'
...
npz_file = '../deep_learning_outputs/model_evaluation/basic_gat_predicted_values_v03.npz'
...
correlation_file = f'../deep_learning_outputs/model_evaluation/basic_gat_correlation_plot_v03.png'
